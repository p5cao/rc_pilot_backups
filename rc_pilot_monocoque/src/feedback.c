...
static rc_filter_t x_acc_lp;
static rc_filter_t y_acc_lp;
...
// define system -DT; // accel bias
	...
	if(rc_filter_first_order_lowpass(&acc_lp, dt, 20*dt)) return -1; //z_acc
    if(rc_filter_first_order_lowpass(&x_acc_lp, dt, 20*dt)) return -1;
    if(rc_filter_first_order_lowpass(&y_acc_lp, dt, 20*dt)) return -1;
