...
int setpoint_manager_update()
{
	...
	case DIRECT_THROTTLE_6DOF:
		...
		setpoint.pitch = 0.0;
		setpoint.X_throttle = -user_input.pitch_stick; 
		setpoint.Y_throttle = user_input.roll_stick + 0.03; //correction according to log data
		setpoint.Z_throttle = -user_input.thr_stick;
		__direct_yaw();
		break;
	...
	case ALT_HOLD_4DOF:
		...
		setpoint.en_rpy_ctrl = 1;
		setpoint.en_6dof = 0;
		setpoint.roll = user_input.roll_stick + 0.03; //correction according to log data
		setpoint.pitch = user_input.pitch_stick;
		__altitude_hold();
		__direct_yaw();
		break;
