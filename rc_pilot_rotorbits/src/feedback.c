...
// low pass filters with 60Hz cutoff freq for roll rate, pitch rate readings;
static rc_filter_t roll_rate_lp, pitch_rate_lp;
...
// define system -DT; // accel bias
	...
	// initialize LP filters on roll_rate and pitch_rate, cut-off freq 60Hz
	if(rc_filter_butterworth_lowpass(&roll_rate_lp, 2, dt, 2.0)) return -1;
	if(rc_filter_butterworth_lowpass(&pitch_rate_lp, 2, dt, 2.0)) return -1;
...
static int __feedback_state_estimate()
{
	...
	if(settings.enable_magnetometer){
		fstate.roll  = mpu_data.fused_TaitBryan[TB_PITCH_X];
		fstate.pitch = -mpu_data.fused_TaitBryan[TB_ROLL_Y];
		yaw_reading  = mpu_data.fused_TaitBryan[TB_YAW_Z];

	}
	else{
		fstate.roll  = mpu_data.dmp_TaitBryan[TB_PITCH_X];
		fstate.pitch = -mpu_data.dmp_TaitBryan[TB_ROLL_Y];
		yaw_reading  = mpu_data.dmp_TaitBryan[TB_YAW_Z];
	}

	//fstate.roll_rate = mpu_data.gyro[1];// for beaglebone facing forward
	fstate.roll_rate = rc_filter_march(&roll_rate_lp,mpu_data.gyro[0]);
	fstate.pitch_rate = rc_filter_march(&pitch_rate_lp,-mpu_data.gyro[1]);
...
static int __feedback_control()
{
	...
	***************************************************************************/
	...
	if(setpoint.en_alt_ctrl){
		...
		//PD controller u_z = (-Kdd*z_2dot - Kd*z_dot(i) - Kp*(z(i) - z_des))/cos(fstate.roll)*cos(fstate.pitch)
		tmp = -1.8*(-setpoint.altitude+fstate.altitude_kf) - 0.7*fstate.alt_kf_vel - 0.01*fstate.alt_kf_accel;
	...
	***************************************************************************/
	if(setpoint.en_rpy_ctrl){
		...
		sp_roll_rate = 5.0 * (setpoint.roll - fstate.roll);
		u[VEC_ROLL] = rc_filter_march(&D_roll, sp_roll_rate - fstate.roll_rate );
		...
		sp_pitch_rate = 5.0 * (setpoint.pitch - fstate.pitch);
		u[VEC_PITCH] = rc_filter_march(&D_pitch, sp_pitch_rate - fstate.pitch_rate);
	...
	if(setpoint.en_6dof){
		...
		//attempting to topup Y_throttle since beagleMAV has little left-right thrust
		u[VEC_Y] = 1.5*setpoint.Y_throttle;
